import { assertRoughlyEqualValues } from "@1inch/solidity-utils";
import { TransactionResponse } from "ethers";
import { ethers } from "hardhat";
import { abi as IOracleAbi } from "../artifacts/contracts/interfaces/IOracle.sol/IOracle";
import { abi as OffchainOracleAbi } from "../artifacts/contracts/OffchainOracle.sol/OffchainOracle";
import {
    Address,
...
export const deployParams = {
    ...
    VelocimeterV2: {
        ...
        initcodeHash:
            "0x0ccd005ee58d5fb11632ef5c2e0866256b240965c62c8e990c0f84a97f311879",
    },
    VelodromeV2: {
        // optimistic network
    ...
    Slipstream: {
        ...
        initcodeHash:
            "0x339492e30b7a68609e535da9b0773082bfe60230ca47639ee5566007d525f5a7",
...
export type DeployParamsKeys = keyof typeof deployParams;

export async function measureGas(tx: TransactionResponse, comment: string) {
    const receipt = await tx.wait();
    console.log("gasUsed", comment, receipt?.gasUsed.toString());
}

function _parseTokenForTestRate(token: Token | [Token, Token]) {
...
async function _getDecimals(token: Address): Promise<bigint> {
    if (token === tokens.ETH || token === tokens.EEE) {
        return 18n;
    }
...
export async function testRate(
    srcToken: Address | [Address, Address],
    dstToken: Address | [Address, Address],
    connector: Address,
    targetOracle: StrictBaseContract<typeof IOracleAbi>,
    referenceOracle?: StrictBaseContract<typeof IOracleAbi>,
    relativeDiff = 0.05,
    thresholdFilter = defaultValues.thresholdFilter
) {
    const { targetToken: targetSrcToken, referenceToken: referenceSrcToken } =
        _parseTokenForTestRate(srcToken);
    const { targetToken: targetDstToken, referenceToken: referenceDstToken } =
        _parseTokenForTestRate(dstToken);

    let { rate: actual } = await targetOracle.getRate(
        targetSrcToken,
        targetDstToken,
        connector,
        BigInt(thresholdFilter)
    );

    const res = await referenceOracle?.getRate(
        referenceSrcToken,
        referenceDstToken,
        connector,
        BigInt(thresholdFilter)
    ...
    let expected = res?.rate ?? BigInt(0);
...
export async function testRateOffchainOracle(
    srcToken: Address,
    dstToken: Address,
    oldOffchainOracle: StrictBaseContract<typeof OffchainOracleAbi>,
    newOffchainOracle: StrictBaseContract<typeof OffchainOracleAbi>,
    relativeDiff = 0.05,
    thresholdFilter = defaultValues.thresholdFilter
) {
    const { targetToken: targetSrcToken, referenceToken: referenceSrcToken } =
    ...
    const actualRate = await newOffchainOracle.getRateWithThreshold(
        targetSrcToken,
        targetDstToken,
        true,
        thresholdFilter
    ...
    const expectedRate = await oldOffchainOracle.getRateWithThreshold(
        referenceSrcToken,
        referenceDstToken,
        true,
        thresholdFilter
    ...
    assertRoughlyEqualValues(
        actualRate.weightedRate,
        expectedRate.weightedRate,
        relativeDiff
    );
}
