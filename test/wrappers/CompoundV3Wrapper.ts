...
import { CompoundV3WrapperAbi } from "~/artifacts/contracts/wrappers/CompoundV3Wrapper.sol/CompoundV3Wrapper.js";
...
describe("CompoundV3Wrapper", function () {
    async function initContracts() {
        ...
        const compoundV3Wrapper = await deployContract<CompoundV3WrapperAbi>(
            "CompoundV3Wrapper",
        ...
        const cWETHv3 = new ethers.Contract(
            tokens.cWETHv3,
            [...IERC20ABI, ...COMETABI],
            wallet
        );
    ...
    it("should revert when add/remove by non-owner", async function () {
        const { nonOwner, compoundV3Wrapper } = await loadFixture(
            initContracts
        );

        await expect(
            compoundV3Wrapper.connect(nonOwner).addMarkets([tokens.cUSDCv3])
        ...
        await expect(
            compoundV3Wrapper.connect(nonOwner).removeMarkets([tokens.cWETHv3])
        ).to.be.revertedWithCustomError(
            compoundV3Wrapper,
            "OwnableUnauthorizedAccount"
        );
    ...
    it("addMarkets", async function () {
        ...
        expect(await compoundV3Wrapper.cTokenToToken(tokens.cUSDCv3)).to.equal(
            tokens.USDC
        );
        expect(await compoundV3Wrapper.tokenTocToken(tokens.USDC)).to.equal(
            tokens.cUSDCv3
        );
    });
    ...
    it("removeMarkets", async function () {
        ...
        expect(await compoundV3Wrapper.cTokenToToken(tokens.cWETHv3)).to.equal(
            constants.ZERO_ADDRESS
        );
        expect(await compoundV3Wrapper.tokenTocToken(tokens.WETH)).to.equal(
            constants.ZERO_ADDRESS
        );
    });

    it("WETH -> cWETHv3", async function () {
        const { wallet, compoundV3Wrapper, cWETHv3 } = await loadFixture(
            initContracts
        );
        ...
        const [received] = await trackReceivedTokenAndTx(
            ethers.provider,
            cWETHv3 as unknown as Token,
            wallet.address,
            async () => cWETHv3.supply(tokens.WETH, ether("1"))
        );
    ...
    it("cWETHv3 -> WETH", async function () {
        const { wallet, compoundV3Wrapper, weth, cWETHv3 } = await loadFixture(
            initContracts
        );
        ...
        const [received] = await trackReceivedTokenAndTx(
            ethers.provider,
            weth as unknown as Token,
            wallet.address,
            async () => cWETHv3.withdraw(tokens.WETH, ether("1"))
        );
