...
describe("Erc4626Wrapper", function () {
    ...
    it("should add market via addMarkets and remove market via removeMarkets", async function () {
        ...
        expect(await erc4626Wrapper.wbaseToBase(tokens.sUSDe)).to.equal(
            tokens.USDe
        );
        expect(await erc4626Wrapper.baseToWbase(tokens.USDe)).to.equal(
            tokens.sUSDe
        );
    ...
    it("should not add market by non-owner", async function () {
        ...
        await expect(
            erc4626Wrapper.connect(alice).addMarkets([tokens.sUSDe])
    ...
    it("should not remove market by non-owner", async function () {
        const { alice, erc4626Wrapper } = await loadFixture(initContracts);
        await expect(
            erc4626Wrapper.connect(alice).removeMarkets([tokens.sUSDe])
        ).to.be.revertedWithCustomError(
            erc4626Wrapper,
            "OwnableUnauthorizedAccount"
        );
    ...
    it("should not return result in wrap method for unsupported token", async function () {
        const { erc4626Wrapper } = await loadFixture(initContracts);
        await expect(
            erc4626Wrapper.wrap(tokens.WETH)
        ).to.be.revertedWithCustomError(erc4626Wrapper, "NotSupportedToken");
    });

    type InitContractsAndMarketFun = Awaited<Record<string, any>>;

    function shouldReturnCorrectPricesAndTokens<
        F extends InitContractsAndMarketFun
    >(fixture: () => Promise<F>) {
        it("base -> wbase", async function () {
            ...
            expect(response.rate).to.equal(
                await wbase.convertToShares(ether("1"))
            );
        ...
        it("wbase -> base", async function () {
            ...
            expect(response.rate).to.equal(
                await wbase.convertToAssets(ether("1"))
            );
    ...
    describe("scrvUSD Wrapper", function () {
        async function initContractsAndMarket() {
            const { erc4626Wrapper } = await initContracts();
            const scrvUSD = await ethers.getContractAt(
                "IERC4626",
                tokens.scrvUSD
            );
