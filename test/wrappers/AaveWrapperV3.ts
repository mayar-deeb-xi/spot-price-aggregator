...
describe("AaveWrapperV3", function () {
    async function initContracts() {
        const aaveWrapper = await deployContract<AaveWrapperV3Abi>(
            "AaveWrapperV3",
            [AaveWrapperV3.lendingPool]
        );
    ...
    it("should return all reserves tokens", async function () {
        const tokens = await getAllAave3ReservesTokens(
            AaveWrapperV3.lendingPool
        );
    ...
    it("should revert with non-supported token", async function () {
        ...
        await expect(aaveWrapper.wrap(token)).to.be.revertedWithCustomError(
            aaveWrapper,
            "NotSupportedToken"
        );
    });
    ...
    it("should correct add market", async function () {
        const { aaveWrapper } = await loadFixture(initContracts);
        const aaveV3Reserves = await getAllAave3ReservesTokens(
            AaveWrapperV3.lendingPool
        );
    ...
    it("should revert if added market is incorrect", async function () {
        const { aaveWrapper } = await loadFixture(initContracts);
        await expect(
            aaveWrapper.addMarkets([tokens.aDAIV1])
        ).to.be.revertedWithCustomError(aaveWrapper, "NotAddedMarket");
    });

    it("should revert if one of added markets is incorrect", async function () {
        const { aaveWrapper } = await loadFixture(initContracts);
        await expect(
            aaveWrapper.addMarkets([tokens.DAI, tokens.aDAIV1])
        ).to.be.revertedWithCustomError(aaveWrapper, "NotAddedMarket");
    });
    ...
    it("should correct remove market which not support by protocol", async function () {
        ...
        await expect(
            aaveWrapper.wrap(tokens.aDAIV1)
        ).to.be.revertedWithCustomError(aaveWrapper, "NotSupportedToken");
    });

    it("should revert if removed market is supported by protocol", async function () {
        const { aaveWrapper } = await loadFixture(initContracts);
        await expect(
            aaveWrapper.removeMarkets([tokens.DAI])
        ).to.be.revertedWithCustomError(aaveWrapper, "NotRemovedMarket");
    });
