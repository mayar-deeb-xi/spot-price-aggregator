...
import { StataTokenWrapperAbi } from "~/artifacts/contracts/wrappers/StataTokenWrapper.sol/StataTokenWrapper.js";
import { getAllAaveV3UnderlyingTokensForStataTokens } from "../../deploy/utils.js";
import { deployParams, tokens } from "../helpers.js";

const { StataTokenWrapper } = deployParams;
...
describe("StataTokenWrapper", function () {
    ...
    async function initContracts() {
        const stataTokenWrapper = await deployContract<StataTokenWrapperAbi>(
            "StataTokenWrapper",
            [StataTokenWrapper.staticATokenFactory]
        );
    ...
    it("should return underlying tokens for all stata tokens", async function () {
        const tokens = await getAllAaveV3UnderlyingTokensForStataTokens(
            StataTokenWrapper.staticATokenFactory
        );
    ...
    it("should revert with non-supported token", async function () {
        ...
        await expect(
            stataTokenWrapper.wrap(tokens.CHAI)
        ).to.be.revertedWithCustomError(stataTokenWrapper, "NotSupportedToken");
    });

    it("should correct add market", async function () {
        const { stataTokens, stataTokenWrapper } = await loadFixture(
            initContracts
        );
        await stataTokenWrapper.addMarkets([tokens.DAI]);
        const response = await stataTokenWrapper.wrap(tokens.DAI);
        assertRoughlyEqualValues(
            await stataTokens.DAI.previewDeposit(ether("1")),
            response.rate,
            1e8
        );
    });
    ...
    it("should correct add already added market", async function () {
        ...
        expect(await stataTokens.USDC.previewDeposit(ether("1"))).to.equal(
            response.rate
        );
    });
    ...
    it("should revert if added market is incorrect", async function () {
        const { stataTokenWrapper } = await loadFixture(initContracts);
        await expect(
            stataTokenWrapper.addMarkets([tokens.CHAI])
        ).to.be.revertedWithCustomError(stataTokenWrapper, "NotAddedMarket");
    });

    it("should revert if one of added markets is incorrect", async function () {
        const { stataTokenWrapper } = await loadFixture(initContracts);
        await expect(
            stataTokenWrapper.addMarkets([tokens.DAI, tokens.CHAI])
        ).to.be.revertedWithCustomError(stataTokenWrapper, "NotAddedMarket");
    });
    ...
    it("should revert if removed market is supported by protocol", async function () {
        const { stataTokenWrapper } = await loadFixture(initContracts);
        await expect(
            stataTokenWrapper.removeMarkets([tokens.USDC])
        ).to.be.revertedWithCustomError(stataTokenWrapper, "NotRemovedMarket");
    });
    ...
    it("USDC -> stataUSDC", async function () {
        ...
        const response = await stataTokenWrapper.wrap(tokens.USDC);
        expect(response.rate).to.equal(
            await stataTokens.USDC.previewDeposit(ether("1"))
        );
    ...
    it("stataUSDC -> USDC", async function () {
        ...
        assertRoughlyEqualValues(
            await stataTokens.USDC.previewWithdraw(ether("1")),
            response.rate,
            1e10
        );
    ...
    it("WETH -> stataWETH", async function () {
        ...
        expect(response.rate).to.equal(
            await stataTokens.WETH.previewDeposit(ether("1"))
        );
    ...
    it("stataWETH -> WETH", async function () {
        ...
        assertRoughlyEqualValues(
            await stataTokens.WETH.previewWithdraw(ether("1")),
            response.rate,
            1e10
        );
