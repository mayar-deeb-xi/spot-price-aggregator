...
const { AaveWrapperV2, Uniswap, UniswapV2 } = deployParams;
...
describe("OffchainOracle", function () {
    async function initContracts() {
        ...
        const uniswapOracle = await deployContract("UniswapOracle", [
            Uniswap.factory,
        ]);
        const mooniswapOracle = await deployContract("MooniswapOracle", [
            tokens.oneInchLP1,
        ]);
        const wethWrapper = await deployContract("BaseCoinWrapper", [
            tokens.ETH,
            tokens.WETH,
        ]);
        const aaveWrapperV1 = await deployContract("AaveWrapperV1", []);
        const aaveWrapperV2 = await deployContract("AaveWrapperV2", [
    ...
    describe("built-in connectors", function () {
        async function initContractsAndOffchainOracle() {
            ...
            const expensiveOffchainOracle = await deployContract(
                "OffchainOracle",
                [
                    multiWrapper,
                    [uniswapV2LikeOracle, uniswapOracle, mooniswapOracle],
        ...
        it("WETH -> DAI", async function () {
            const { offchainOracle } = await loadFixture(
                initContractsAndOffchainOracle
            );
            const rate = await offchainOracle.getRateWithThreshold(
                tokens.WETH,
                tokens.DAI,
                true,
                thresholdFilter
        ...
        it("getRate(DAI -> LINK)", async function () {
            ...
            const { expensiveOffchainOracle, gasEstimator } = await loadFixture(
                initContractsAndOffchainOracle
            );
            const result = await gasEstimator.gasCost(
                await expensiveOffchainOracle.getAddress(),
                expensiveOffchainOracle.interface.encodeFunctionData(
                    "getRateWithThreshold",
                    [tokens.DAI, tokens.LINK, true, thresholdFilter]
                )
            );
        ...
        it("getRateToEth(DAI)_ShouldHaveCorrectRate", async function () {
            ...
            const expectedRate = await offchainOracle.getRateWithThreshold(
                tokens.DAI,
                tokens.WETH,
                true,
                thresholdFilter
            );
            const actualRate = await offchainOracle.getRateToEthWithThreshold(
                tokens.DAI,
                true,
                thresholdFilter
            );
            assertRoughlyEqualValues(expectedRate[0], actualRate[0], 1e-2);
        });
        ...
        it("getRateToEth(DAI)", async function () {
            ...
            const result = await gasEstimator.gasCost(
                ...
                expensiveOffchainOracle.interface.encodeFunctionData(
                    ...
                    [tokens.DAI, true, thresholdFilter]
                )
            );
        ...
        it("getRateDirect(DAI -> LINK)_ShouldHaveCorrectRate", async function () {
            ...
            const actualRate = await offchainOracle.getRateWithThreshold(
                tokens.DAI,
                tokens.LINK,
                false,
                thresholdFilter
        ...
        it("getRateDirect(DAI -> LINK)", async function () {
            ...
            const result = await gasEstimator.gasCost(
                ...
                expensiveOffchainOracle.interface.encodeFunctionData(
                    "getRateWithThreshold",
                    [tokens.DAI, tokens.LINK, false, thresholdFilter]
                )
            );
        ...
        it("check getRatesAndWeightsWithCustomConnectors method with non-wrapped price", async function () {
            ...
            const result =
                await offchainOracle.getRatesAndWeightsWithCustomConnectors(
                    tokens.DAI,
                    tokens.LINK,
                    true,
                    [],
        ...
        it("check getRatesAndWeightsToEthWithCustomConnectors method with non-wrapped price", async function () {
            ...
            const result =
                await offchainOracle.getRatesAndWeightsToEthWithCustomConnectors(
                    tokens.DAI,
                    true,
                    [],
                    thresholdFilter
    ...
    describe("customConnectors", function () {
        async function initContractsAndOffchainOracle() {
            ...
            const connectors = [tokens.ETH, tokens.WETH, tokens.USDC];
            const offchainOracle = await deployContract("OffchainOracle", [
                multiWrapper,
                [uniswapV2LikeOracle, uniswapOracle],
                ["0", "1"],
                [tokens.NONE, ...connectors],
                tokens.WETH,
                deployer.address,
            ]);

            const offchainOracleWithoutConnectors = await deployContract(
                "OffchainOracle",
                [
                    multiWrapper,
                    [uniswapV2LikeOracle, uniswapOracle],
            ...
            return {
                offchainOracle,
                offchainOracleWithoutConnectors,
                connectors,
            };
        }
        ...
        it("WETH -> DAI", async function () {
            const {
                ...
                offchainOracleWithoutConnectors,
                connectors,
            } = await loadFixture(initContractsAndOffchainOracle);
            const rateWithCustomConnector =
                await offchainOracleWithoutConnectors.getRateWithCustomConnectors(
                    tokens.WETH,
                    tokens.DAI,
                    true,
                    connectors,
    ...
    describe("Some features", function () {
        it("should work when overflow happens in _getRateImpl method", async function () {
            ...
            const simpleOracleMock = await deployContract("SimpleOracleMock", [
                "608424427628800532964876503129856304465282478",
                "2",
            ]);
            const offchainOracle = await deployContract("OffchainOracle", [
                multiWrapper,
            ...
            expect(await offchainOracle.getRateToEth(tokens.DAI, true)).not.to
                .be.reverted;
        });
        ...
        it("should correct work with wrappers when price is not 1:1", async function () {
            ...
            const wstETHWrapper = await deployContract("WstETHWrapper", [
                tokens.ETH,
                tokens.wstETH,
            ]);
            await multiWrapper.addWrapper(wstETHWrapper);

            const offchainOracle = await deployContract<OffchainOracleAbi>(
                "OffchainOracle",
                [multiWrapper, [], [], [], tokens.WETH, deployer.address]
            );

            const rateForward =
                await offchainOracle.getRateWithCustomConnectors(
                    tokens.WETH,
                    tokens.wstETH,
                    true,
                    [],
            ...
            assertRoughlyEqualValues(
                (rateForward.weightedRate * rateReverse.weightedRate) /
                    BigInt(1e18),
                BigInt(1e18),
                5e-18
            );
        ...
        it.skip("measure gas of current implementation", async function () {
            ...
            const offchainOracleDeployment = JSON.parse(
                fs.readFileSync(
                    "deployments/mainnet/OffchainOracle.json",
                    "utf8"
                )
            );
            const offchainOracle = await ethers.getContractAt(
                "OffchainOracle",
                offchainOracleDeployment.address
            );
            ...
            const getRateToEthResult = await gasEstimator.gasCost(
                ...
                offchainOracle.interface.encodeFunctionData(
                    "getRateToEthWithThreshold",
                    [tokens.DAI, true, thresholdFilter]
                )
            );
            console.log(
                `OffchainOracle getRateToEthWithThreshold(DAI,true,${thresholdFilter}): ${getRateToEthResult.gasUsed}`
            );
            ...
            const getRateResult = await gasEstimator.gasCost(
                ...
                offchainOracle.interface.encodeFunctionData(
                    "getRateWithThreshold",
                    [tokens.USDC, tokens.USDe, true, thresholdFilter]
                )
            );
            console.log(
                `OffchainOracle getRateWithThreshold(USDC,USDe,true,${thresholdFilter}): ${getRateResult.gasUsed}`
            );
