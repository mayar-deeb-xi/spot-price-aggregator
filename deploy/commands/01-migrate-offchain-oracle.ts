...
const func: DeployFunction = async function ({
    ...
    const offchainOracle = OffchainOracle.attach(
        (await deployments.get("OffchainOracle")).address
    );
    ...
    for (let i = 0; i < oracles.allOracles.length; i++) {
        ...
        const contractName =
            OraclesToUpdate[networkName][oracles.allOracles[i]].contract;
        const exchangeName =
        ...
        try {
            ...
            if (OraclesToUpdate[networkName][oracles.allOracles[i]].params) {
                params =
                    OraclesToUpdate[networkName][oracles.allOracles[i]].params;
            } else {
        ...
        if (isNoDeployments) {
            switch (contractName) {
                ...
                case "SolidlyOracle":
                    oracle = await attachContract(
                        contractName,
                        oracles.allOracles[i]
                    );
                    params = [
                        await oracle.factory(),
                        await oracle.initcodeHash(),
                    ];
                    break;
