...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    const proxyAddress = (await deployments.get("TransparentUpgradeableProxy"))
        .address;
    ...
    const upgradeTxn = await admin.upgradeAndCall(
        proxyAddress,
        implAddress,
        "0x"
    );
    ...
    console.log(
        "Proxy for OffchainOracle at",
        proxyAddress,
        "upgraded with new impl at",
        implAddress
    );
};
