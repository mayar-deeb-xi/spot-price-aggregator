...
async function main() {
    ...
    const token =
        process.env.SCRIPT_TOKEN ||
        "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";
    const isConnectorsZeroPrice =
        (process.env.SCRIPT_CONNECTORS_ZERO_PRICE || "false").toLowerCase() ===
        "true";
    const thresholdFilter = process.env.SCRIPT_THRESHOLD_FILTER || 10;

    const allDeployments = await deployments.all();
    const contractNameByAddress: Record<string, string> = {};

    Object.keys(allDeployments).forEach(function (contractName) {
        const address = allDeployments[contractName].address;
    ...
    const currentPrice = await deployedOffchainOracle.getRateToEthWithThreshold(
        token,
        true,
        thresholdFilter
    );
    ...
    for (let i = 0; i < oracles.allOracles.length; i++) {
        ...
        const price = await offchainOracle.getRateToEthWithThreshold(
            token,
            true,
            thresholdFilter
        );
        if (parseFloat(price) !== 0) {
            ...
            for (let j = 0; j < connectors.length; j++) {
                ...
                try {
                    ...
                    const { rate, weight } = await oracle.getRate(
                        token,
                        weth,
                        connectors[j],
                        thresholdFilter
                    ...
                    pricesViaConnector.push({
                        connector: symbol,
                        rate: usdPrice(rate, decimals),
        ...
        await offchainOracle.removeOracle(
            oracles.allOracles[i],
            oracles.oracleTypes[i]
        );
    }
